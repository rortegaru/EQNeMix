...
class eqnefmm:
    def __init__(self, eqnegrid , **kwargs):
        """
        ...
        """
        self.eqnegrid = eqnegrid  # Guarda la instancia de eqnegrid
        self.ngrid = eqnegrid.max_elements
        self.minx,self.miny=self._getminxy()
        self.orig={'minx': self.minx, 
            'miny': self.miny }
        self.velp = np.load(eqnegrid.filep)
        self.vels = np.load(eqnegrid.filep)
        self.vpp = self.velp.reshape([self.ngrid*self.ngrid*self.ngrid,
                                      1], order='F').astype('float32')
        self.vss = self.vels.reshape([self.ngrid*self.ngrid*self.ngrid,
                                      1],order='F').astype('float32')
